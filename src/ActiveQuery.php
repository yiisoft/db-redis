 ...
 *
 * A relation is specified by {@see link} which represents the association between columns
 * of different tables; and the multiplicity of the relation is indicated by {@see multiple}.
 *
...
class ActiveQuery implements ActiveQueryInterface
{
    ...
    public function __construct(string $modelClass, array $config = [])
    {
        ...
        $this->db = $modelClass::getConnection();
    }
     ...
     * query result may be either an array or an ActiveRecord object. Null will be returned.
     ...
     *
     * @param string $column name of the column to select. If this parameter is not given, the `db` application
     ...
     */
    public function average($column): int
    {
        if ($this->emulateExecution) {
            return 0;
        }
     ...
     */
    public function scalar($attribute)
    {
        if ($this->emulateExecution) {
            return null;
        }
    ...
    protected function executeScript($type, $columnName = null)
    {
        ...
        if (
            is_array($this->where)
            && (
                (!isset($this->where[0]) && $this->modelClass::isPrimaryKey(array_keys($this->where)))
                || (isset($this->where[0]) && $this->where[0] === 'in'
                && $this->modelClass::isPrimaryKey((array) $this->where[1])
               )
            )
        ) {
        ...
        $script = $this->db->getLuaScriptBuilder()->$method($this, $columnName);
